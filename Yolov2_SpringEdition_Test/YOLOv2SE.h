...
*
*/
#if !defined(YOLO_7E0_05_17_YOLOV2_H_INCLUDED)
#define YOLO_7E0_05_17_YOLOV2_H_INCLUDED
#include<fstream>
#include<string>
#include<vector>
#include<functional>
...
public:
	...
	YOLOv2() {
#ifdef _WIN32
		std::string dll = "libYOLOv2SE.dll";
		m_hmod = LoadLibraryA(dll.c_str());
		if (m_hmod == nullptr) {
			::MessageBoxA(NULL, "libYOLOv2SE.dll not found. or can't load dependency dll(cudnn64_5)", "Fatal", MB_OK);
			exit(1);
		}
...
#else
		m_hmod = dlopen("libYOLOv2SE.so",RTLD_LAZY);
		if (m_hmod == nullptr) {
			std::cerr << dlerror() << std::endl;
			std::cerr << "libYOLOv2SE.so not found. or can't load dependency dlls" << std::endl;
			exit(1);
		}
